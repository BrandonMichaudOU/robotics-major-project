...
def d_star(graph, start, end):
    ...
    def process_state():
        ...
        k_old = cost_from_goal[min_node]
        closed.append(min_node)

        h_min_node = euclidean_distance(min_node, end)
        if k_old < h_min_node:
            for neighbor in graph.get_connections(min_node):
                h_neighbor = euclidean_distance(neighbor, end)
                if (neighbor in open or neighbor in closed) and h_neighbor < k_old and h_min_node >     h_neighbor + min_node.get_weight(neighbor):
                    backpointer[min_node] = neighbor
